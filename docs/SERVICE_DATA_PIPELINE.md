...
공개 문서 추출 → 수집 보고서 확인 → 업무·분류·조직 대응표 작성 → 검수 대기 등록 → 담당자 승인 → 시민 검색 순서다.
...
`/staff/source-reports`에서 추출 보고서 v2를 열어 누락·중복·부서 표기·출처 확인 항목을
필터링하고 페이지 근거를 볼 수 있다. 앱 서버에서 검사하지만 파일·검수 결과를 저장하지 않으며
시민 검색이나 승인 이력에 반영하지 않는다. 팀 전달용 확인할 일 JSON도 검수 대기 자료다.
[화면 사용 방법과 검증 범위](SOURCE_REPORT_REVIEW.md)를 따른다.
