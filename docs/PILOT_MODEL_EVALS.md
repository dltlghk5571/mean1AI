...
2026-09-13 [가로등·관련 경계 14건의 계약 대조](LIGHTING_LABEL_REVIEW.md)를 추가했다.
LLR-01의 안내 요청에 접수 질문이 붙는 앱 흐름은 수정했다. LLR-02의 UI 이어보기·수정에서는
답변·건너뛰기 손실이 재현되지 않았고 회귀 검증을 추가했다. LLR-03은 pc027·pc013의 인용 후보
8개를 대조해 사례별 정확한 인용 대안을 권고했다. LLR-04는 pc005·pc072의 장소 제안 5개와 합성
후속 상태 흐름 6개를 대조하고 모호한 단서를 원문에 보존한 채 장소를 물을 기준안을 작성했다.
LLR-05는 혼합·범위 밖 네 사례의 API·실행 계약을 대조해 다음 안내 기준안을 작성했다. 평가의
clarify/handoff와 앱의 목적 질문이 다르고, pc083은 urgent=false 초안과 달리 위험 규칙이
추출을 건너뛰게 한다. LLR-06은 부정·사칭 세 사례의 수동 제안·동의·계획·분류를 대조했다.
원문에 있는 잘못된 근거와 등록된 엉뚱한 후보는 형식 검사를 통과할 수 있지만 접수·권한 통제는
별도로 적용됨을 기록했다. 기존 상태 대응 합의, LLR-03~06 최종 결정과 실제 모델 평가는 대기다.
현재 정답·분할·버전은 그대로이며 이 기록은 모델 평가나 A·D의 최종 승인이 아니다.
