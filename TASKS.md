...
## 마지막 수행 · G3.1 LLR-06 부정·사칭 문장의 검수 기준
...
## 다음 한 가지 · G2.1 쓰레기 무단투기 공식 근거 검수

- 상위 목표: G2 성남 업무에 맞는 근거 → G0 시민이 부서를 몰라도 현장 불편을 설명하고 안내받음.
- 결과물: 기존 [공식 자료 작업표](docs/SEONGNAM_DATA_REVIEW.md)에 `pilot-service-dumping`
  한 건의 담당 업무·신고 진입점·필요 정보와 미확인 범위를 대조한 기록.
- 완료 증거: 세 구 공식 업무표와 신고 안내의 원문·조회 시각·이용 표시를 확인하고,
  쓰레기 수거/현장 확인과 행위자 조사·처분을 구분. 현재 후보의 부서·질문이 실제 근거와
  맞는지 확인/미확인을 남기고 A·C의 적용 전 결정 사항을 기록.
- 제외: 처벌·위반 여부 판단, 주민 정보 수집·로그인 우회·기관 문의 발송, 다른 업무로 수집 확대,
  후보 JSON·실자료 승인·자동 배정 변경, 모델·UI·새 수집 플랫폼 구현.
- 의존성·종료: 공개 자료는 서버·참여자 없이 읽기 전용으로 대조. 비공개 신고서·이용 범위·관할을
  확인하지 못하면 필요한 근거와 A·C 담당을 남기고 한 업무의 대조에서 종료한다.
