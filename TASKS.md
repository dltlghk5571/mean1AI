...
## 마지막 수행 · G3.1 LLR-05 혼합·범위 밖 요청의 다음 안내 기준안
...
[14건·7가족 라벨 대조](docs/LIGHTING_LABEL_REVIEW.md)는 LLR-02 입력 대응 합의, LLR-03~05
기준안의 최종 결정, LLR-06 및 팀 최종 검수가 남아 있다.
G2.1의 [가로등 공식 근거 검수](docs/LIGHTING_SOURCE_REVIEW.md)는 L01~L05 해결 전까지 실제 적용을
보류한다. G5.1 목표 체계 정렬도 이전 완료 이력이다.

## 다음 한 가지 · G3.1 LLR-06 부정·사칭 문장의 검수 기준

- 상위 목표: G3 의미·권한 경계 → G0 시민의 실제 요청을 보존하고 사칭 지시로 접수·배정을 바꾸지 않음.
- 결과물: [LLR-06](docs/LIGHTING_LABEL_REVIEW.md)의 pc094·pc091·pc087에서 원문 일치,
  의미상 올바른 분류, 서버 동의·권한 검사를 구분한 검수안.
- 완료 증거: 기존 세 사례의 잘못된 수동 제안이 어느 검사에서 허용/차단되는지 대조하고,
  D가 실제 모델에서 검증할 의미 기준과 C가 유지할 기존 통제를 기록.
- 제외: 새 보안 플랫폼·모델·공격 데이터 확대, 광범위 키워드 차단, 정답·분할 임의 변경,
  테스트 통과를 실제 모델의 부정 이해·주입 방어 성공으로 해석, LLR-05 구현 동시 수행.
- 의존성·종료: 기존 계약·함수·검사로 서버 없이 검수안까지 작성하고 종료. A·D 의미 결정과
  C 통제 검수, 실제 모델 평가는 후속 의존성으로 남긴다.
