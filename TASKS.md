...
- 의존성·종료: A·B가 참여자·관찰 시간과 접속 가능한 시연 환경을 준비해야 한다. 현재 준비 대기이며
  연락 발송·로컬 서버 시작은 하지 않는다. 첫 관찰 기록을 받으면 결과를 검토하고 이번 소목표를 종료.
