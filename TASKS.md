...
## 마지막 수행 · G3.1 LLR-02 답변·건너뛰기 상태 대조
...
[14건·7가족 라벨 대조](docs/LIGHTING_LABEL_REVIEW.md)는 LLR-02 입력 대응 합의, LLR-03~06 및
A·D 최종 검수가 남아 있다.
G2.1의 [가로등 공식 근거 검수](docs/LIGHTING_SOURCE_REVIEW.md)는 L01~L05 해결 전까지 실제 적용을
보류한다. G5.1 목표 체계 정렬도 이전 완료 이력이다.

## 다음 한 가지 · G3.1 LLR-03 인용 범위 검수안

- 상위 목표: G3 모델 입력·평가 정답 → G0 시민이 말한 시점을 정확히 보존하는지 평가.
- 결과물: [LLR-03](docs/LIGHTING_LABEL_REVIEW.md)의 pc027·pc013에서 원문 인용과 허용 정답의
  차이를 비교한 검수안. `사흘 전 밤`과 `사흘 전 밤부터` 등 후보를 사례별로 제시.
- 완료 증거: 후보의 원문 일치·의미 차이·현행 채점 결과를 대조하고 A·D가 결정할 선택지를 기록.
  검수 전 정답·분할·버전을 바꾸거나 실제 모델 오류로 집계하지 않음.
- 제외: 날짜 추론·범용 문자열 정규화, 새 모델·평가 플랫폼, LLR-04~06 동시 수행.
- 의존성·종료: 기존 합성 사례와 채점기로 서버·참여자 없이 검수안 작성 가능. 사례별 근거를
  기존 문서에 기록하면 종료하며, 허용 정답을 바꾸는 최종 결정은 A·D 검수 후 별도 수행.
...
| G3.1 모델 입력·평가 정답 확정 | 기존 JSON 계약과 합성 라벨의 불일치를 검토한 버전·기록 | 14건 대조·LLR-01 수정·LLR-02 UI 대조 완료. 다음 LLR-03 검수안, 입력 대응과 A·D 최종 합의는 별도 | D·A |
...
LLR-02의 입력 대응과 LLR-03~06은 G3.1의 후속 후보다. 이번 UI 대조의 완료를 막지 않으며,
입력/행동 대응과 라벨 기준을 A·D·C가 합의할 때 해당 변경만 소목표로 선택한다.
LLR-01의 앱 수정과 LLR-02의 UI 상태 검증은 완료했다.
