...
- 의존성·종료: 공개 대조는 종료. PG01~05의 담당·시설 범위·질문·이용·적용은 A·C 검수 대기.
  같은 검색을 반복하지 않으며 다음은 가로수 가지·통행 불편 한 업무의 담당·관리 범위를 확인한다.
...
## 다음 한 가지 · G2.1 가로수 가지·통행 불편의 담당·관리 범위 검수

- 상위 목표: G2 성남 업무에 맞는 근거 → G0 시민이 나무 종류·관리기관을 몰라도 통행 불편을 설명.
- 결과물: 기존 [공식 자료 작업표](docs/SEONGNAM_DATA_REVIEW.md)에 `pilot-service-street-tree`
  한 업무의 세 구 담당 후보, R04 녹지관리 지역과 도로변/공원/사유지 범위, 후보 질문의 대조 기록.
- 완료 증거: 공식 업무표·관리/신고 안내의 실제 조회 시각, 가로수와 다른 수목의 확인/미확인
  범위를 구분하고 A·C의 적용 전 결정 기록.
- 제외: 수목 전수 목록·좌표 색인, 사유 시설 책임 추정·수목 진단·직접 가지치기 안내, 다른 업무 확대,
  후보 JSON·앱·자료 승인·위험 규칙 변경, 로그인·실제 신고·기관 연락·새 플랫폼.
- 의존성·종료: 공개 자료를 읽기 전용으로 대조. 수목별 관할·관리 경계·신고 요건을 확정하지
  못하면 필요한 추가 근거와 담당을 남기고 한 업무에서 종료. 로컬 서버는 시작하지 않음.
...
| G2.1 대표 업무 자료 검수 | 한 번에 한 업무의 근거를 확인하고 12개 후보의 검수 상태를 누적 | 가로등·도로·무단투기·기초연금·활동지원·기초생활보장·긴급복지·하수도·놀이터 9/12개 공개 대조 및 RD02 추가 대조 완료. 이용 범위·신고서·관할·정책 시점·하단 역할 확정은 추가 근거와 A·C 검토 대기 | A·C |
