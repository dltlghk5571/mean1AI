...
[14건·7가족 라벨 대조](docs/LIGHTING_LABEL_REVIEW.md)는 LLR-02 입력 대응 합의, LLR-03 검수안의
최종 결정, LLR-04~06 및 A·D 최종 검수가 남아 있다.
G2.1의 [가로등 공식 근거 검수](docs/LIGHTING_SOURCE_REVIEW.md)는 L01~L05 해결 전까지 실제 적용을
보류한다. G5.1 목표 체계 정렬도 이전 완료 이력이다.

## 다음 한 가지 · G3.1 LLR-04 모호한 장소의 처리 기준

- 상위 목표: G3 입력·평가 기준 → G1·G0 정확한 주소를 몰라도 부족한 장소 단서를 보완할 수 있음.
- 결과물: [LLR-04](docs/LIGHTING_LABEL_REVIEW.md)의 pc005·pc072에서 `골목`·`우리 골목` 같은
  원문 단서와 위치를 특정할 수 있는 답변을 구분한 기준안. 현행 장소 확인 흐름과 정답을 대조.
- 완료 증거: 각 후보의 원문 일치·정답 일치·적용 후 질문을 확인하고, 모호할 때 묻거나 건너뛰게
  하는 선택의 근거를 기록. 단순 인용 통과를 위치가 충분하다는 증거로 쓰지 않음.
- 제외: 지오코딩·주소 정규화·시설 색인·새 모델 연결, 정답 임의 변경, LLR-05~06 동시 수행.
- 의존성·종료: 기존 원문·함수로 서버·참여자 없이 대조 가능. 기존 문서의 기준안까지 완료하고
  의미 라벨과 UI 적용의 최종 합의는 A·B·D 검수로 남긴다. 범용 위치 플랫폼으로 확장하지 않음.
...
| G3.1 모델 입력·평가 정답 확정 | 기존 JSON 계약과 합성 라벨의 불일치를 검토한 버전·기록 | 14건 대조·LLR-01 수정·LLR-02 UI 대조·LLR-03 검수안 완료. 다음 LLR-04 기준안, 입력 대응·정답 최종 결정은 별도 | D·A |
...
LLR-02의 입력 대응, LLR-03 정답 반영과 LLR-04~06은 G3.1의 후속 후보다. 이번 검수안의 완료를
막지 않으며, 입력/행동 대응과 라벨 기준을 팀이 합의할 때 해당 변경만 소목표로 선택한다.
LLR-01 앱 수정, LLR-02 UI 상태 검증과 LLR-03 인용 검수안은 완료했다.
