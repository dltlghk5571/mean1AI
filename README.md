...
디지털 행정에 익숙하지 않은 시민도 부서명과 복잡한 서식을 몰라도 대화로 필요한 안내를
받거나 민원을 준비·확인·접수하고 진행을 확인하는 서비스가 목표입니다. 시민 챗봇을 메인으로,
검수된 정보·대화 에이전트·분류 LLM·담당자 검토를 연결해 시민의 탐색과 직원의 반복 업무를 줄입니다.
현재는 앱 내부 접수와 합성 자료 중심의 프로토타입이며 실제 모델·공식 자료·사용자 효과 검증은 남아 있습니다.

**작업을 시작할 때:** [대목표·중간목표·완료 기준](GOALS.md) → [이번 소목표·다음 우선순위](TASKS.md)
순서로 확인합니다. 새 기능 수와 테스트 수를 제품 목표의 달성으로 보지 않습니다.

**팀 공유·제출 준비:** [구현 현황](docs/PROJECT_STATUS.md) ·
[일반부 제안서 원고](docs/PROJECT_PROPOSAL.md) · [첨부 평가 가이드 검토](docs/CHAT_EVAL_GUIDELINE_REVIEW.md) ·
[공고·제출 기준](docs/CONTEST_SUBMISSION_NOTES.md).
제안서는 정식 서식의 항목 순서에 맞춘 MD 원고이며, HWPX 편집·최종 페이지 검증은 별도입니다.
...
- 생활불편·복지 12개 상황의 추가 질문 24개, 답변 이어보기·건너뛰기·개별 수정·긴급 안내
- 사진 최대 3장 선택·미리보기·삭제, 최종 동의 시 함께 저장, 시민·담당자 비공개 사진 조회
- 기존 민원에 추가 문의, 담당자 미답변 대기 목록·직접 답변 공개, 재시도 중복 저장 방지
...
- 담당자가 확인한 민원의 공통 현장 사건 연결·분리, 진행 관리와 시민용 공통 안내 공개·철회
- 선택적 LLM 같은 사건 비교 계약·근거 검증·담당자 화면, 기본 미연결 및 합성 시연 모드
...
- 동아리 분류 모델 HTTP 연결·지연 처리 큐, 카탈로그·모델 버전 검증과 필수 담당자 검토
- 기본 비활성 모델 API 팩터리: 대화·입력 정리·분류·사건 비교와 명시적 합성 응답, 실제 추론 주입 위치
- 자유 입력의 목적·상황·장소·답변 제안 연결부, 원문 인용 확인 카드와 시민 확인 후 반영
...
- [목표 체계와 작업 범위](GOALS.md): 시민 중심 대목표, 중간목표 5개, MVP 완료 증거와 작업 종료 기준.
- [현재 소목표와 다음 작업](TASKS.md): 담당 역할·의존성·보류 목록, 기존 기술 구현 이력.
...
`club`은 별도 설정을 요구하는 전용 HTTP 대화 모델 어댑터입니다. 접수 후 분류에도 별도
`AI_PROVIDER=club` 연결부와 모델 API 서버 골격을 준비했습니다. 실제 동아리 서버는 미연결이며
자유 입력 정리 제안과 확인 UI도 구현했으며 기본은 `CHAT_EXTRACTION_PROVIDER=off`입니다.
`demo`는 예시 문장 하나에 고정된 결과를 보여주며, 실제 모델 연결·평가·공식 데이터 최종 검수·
사진 AI 판독·기관 접수는 후속 범위입니다.
...
- [공통 현장 사건 연결·분리·진행 안내와 권한](docs/SHARED_INCIDENTS.md)
- [LLM 같은 사건 비교 JSON 규격·서버 설정·담당자 검토](docs/INCIDENT_COMPARISON.md)
...
- [동아리 분류 모델 JSON·설정·지연 처리·합성 모드](docs/CLUB_CLASSIFIER.md)
...
확인된 후보에서 **사건 연결 검토**를 누르면 같은 현장 업무를 만들거나 기존 사건에 연결할 수
있습니다. **공통 현장 사건** 메뉴에서 연결 민원 수·진행을 관리하고 오연결을 분리합니다.
검토 승인 계정이 공통 안내를 명시적으로 공개하면 해당 민원 조회 권한이 있는 시민만 볼 수
있습니다. 연결·분리·상태 변경 시 안내는 재검토 전까지 숨겨집니다. [범위와 시연 순서](docs/SHARED_INCIDENTS.md)를 참고하세요.

후보의 **같은 사건 비교**에서는 두 민원을 나란히 검토하고 선택적으로 LLM 제안을 요청합니다.
`INCIDENT_COMPARE_PROVIDER=off`가 기본값이며 `demo`는 실제 판단 없이 합성 응답만 표시합니다.
동아리 분류 LLM의 비교 작업은 `club` 설정으로 연결할 수 있습니다. 입력에 없는 근거·잘못된
응답을 거부하고 내용이 바뀌면 결과를 숨깁니다. 실제 모델 연결·성능 평가는 남아 있습니다.
[JSON 규격과 연결 방법](docs/INCIDENT_COMPARISON.md)을 모델·서버 담당자에게 전달하세요.
...
동아리 분류의 `AI_PROVIDER=club`은 이 큐를 필수로 사용하며 [별도 설정](docs/CLUB_CLASSIFIER.md)을 따릅니다.
...
오류로 처리합니다. 큐용 서버·브로커·외부 저장소는 추가하지 않습니다. `openai` 또는 `club`을
명시적으로 설정한 경우에만 해당 분류 제공자를 호출하며 실제 정부 시스템·메시징 연계는 없습니다.
...
안내 목적의 생활불편 주제는 접수 문항 없이 자료 검색으로 이어지도록 수정했습니다. 주제와 이미
알려준 내용은 보존하며, 시민이 접수로 전환하면 부족한 항목만 묻습니다. 라벨 최종 검수는 대기입니다.
...
GET  /api/v1/session
GET  /api/v1/source-reports/example
POST /api/v1/source-reports/preview
