...
- 시민 메인(`/`), 민원 작성·미리보기·접수 완료, 비공개 내 민원 조회
...
챗봇 UI·동아리 모델 서버·공식 민원 접수 연계는 계획 단계입니다. 현재 구현 범위는 위의
...
1. 시민 메인에서 `민원 신청하기`를 누르고 빈 양식의 `합성 예시 넣기`로 체험합니다.
2. `내용 확인하기`에서 비식별 미리보기를 확인한 뒤 데모 안내에 동의하고 접수합니다.
   미리보기는 저장하거나 AI를 호출하지 않습니다. 원문은 최종 접수 시 로컬 DB에 저장됩니다.
